...
-- postulate ∀≡ : ∀{l k} → {A : Set l} → {B : Set k} → {f g : A → B} → ((x : A) → f x ≡ g x) → f ≡ g
...
[_]and[_] : ∀{l₁ l₂} {A : Set l₁} {P : A → Set l₂} {x y : A} → x ≡ y → P y → P x
[_]and[_] refl px = px
...
_under_ : ∀{l k} → {A : Set l} → {B : Set k} → {x y : A} → x ≡ y → (f : A → B) → f x ≡ f y
_under_ refl f = refl
...
_⇔⟨_⟩_ : (a : Bool) → {b c : Bool} → [ a ⇔ b ] → [ b ⇔ c ] → [ a ⇔ c ]
_⇔⟨_⟩_ a {b} {c} a⇔b b⇔c = ∧-intro (⇒-intro (λ a → b→c (a→b a))) (⇒-intro λ c → b→a (c→b c))
                   where a→b : [ a ] → [ b ]
                         a→b = ⇒-elim (∧-elim₁ a⇔b)
                         b→a : [ b ] → [ a ]
                         b→a = ⇒-elim (∧-elim₂ a⇔b)
